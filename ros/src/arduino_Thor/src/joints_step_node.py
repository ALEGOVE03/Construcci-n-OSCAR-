...
#funciones para obtener los pasos necesarios de la posicion actual a la deseada
# para la articualcion 5
def ART5_motor(j5):
...
def get_joints(data): #funcion que se ejecuta cuando recibe la posicion de las articulaciones en radiades
    ...
    # luego lo pasa a grados y lo multiplica por la constante que pasa de grados a pasos
    ...
    # cada articulacion en pasos se redondea y se vuelve un entero, luego se asigna al menssaje joint
    # para ser puplicado al arduino, ademas se debe tomar en cuenta el comportamiento del sistema
    # diferencial de la articualcion 5 y 6, como du movmiento depende de dos motores se hace de la forma
    # mostrada, primero el movimiento del 5 y luego del 6, ya que si se hace simultaneo no funciona
    ...
    # como el movimiento depende de los mismos dos motores se trabaja con dos variables globales
    # y simplimente se suma o resta los pasos que correspondan
...
if __name__ == '__main__':
    ...
    rospy.init_node('Thor_arduino_actuadores', anonymous=True)

    # se define jointStep para publicar en el topic  joints_steps
    jointStep = rospy.Publisher('joints_steps',joints_steps, queue_size=500)

    # se define grippercmd para pubicar en el topic gripper
    grippercmd = rospy.Publisher('gripper',GripperCommand, queue_size=100)

    #se define el subscriptor donde se obtienen las posiciones de las articualciones
    # del topic '/move_group/fake_controller_joint_states' el cual ofrece moveit
    rospy.Subscriber('/move_group/fake_controller_joint_states', JointState, get_joints )
